  ...
  return (
    <div className="m-page-fade">
      ...
      <div className="page-hd m-hide">
        <h1>{isEdit?'Edit Trade':'Add New Trade'}</h1>
        <div style={{display:'flex',gap:12}}>
          ...
          <button className="btn btn-primary" onClick={()=>save(true)} disabled={missing.length>0||score<75||busy}>
            Save Final Trade
          </button>
        </div>
      </div>
      ...
      <div className="page-hd d-hide">
        ...
        <div style={{display:'flex',gap:8}}>
          ...
          <button className="m-glass-btn" onClick={()=>save(true)} disabled={missing.length>0||score<75||busy} 
                  style={{ fontSize: '12px', padding: '8px 12px', background: (missing.length>0||score<75) ? 'transparent' : 'var(--m-primary)', borderColor: (missing.length>0||score<75) ? 'var(--m-border)' : 'var(--m-primary)', color: '#fff' }}>
            Final
          </button>
        </div>
      </div>
      ...
      <div className="m-hide">
        <div className="card">
          <div className="form-sec">Basic Info</div>
          ...
          <div className="g2">
            <div className="field">
              ...
              <input value={pair} onChange={e=>setPair(e.target.value.toUpperCase())} placeholder="e.g. EURUSD"/>
            </div>
            <div className="field">
              <label>Date</label>
            ...
            <div className="field">
              <label>Risk %</label>
              <input type="number" value={risk} onChange={e=>setRisk(e.target.value)} placeholder="0.5"/>
            </div>
          </div>
        </div>

        <div className="card">
          <div className="form-sec">Setup Checklist</div>
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 24px'}}>
            {activeItems.map(item=>(
              <div key={item.key} className={`ci ${cl[item.key]?'on':''}`} onClick={()=>toggle(item.key)}
                   style={{'--ici':WEIGHT_COLORS[item.weight]?.color||DEFAULT_COLOR.color, '--ibg':WEIGHT_COLORS[item.weight]?.bg||DEFAULT_COLOR.bg, '--irgb':WEIGHT_COLORS[item.weight]?.rgb||DEFAULT_COLOR.rgb}}>
                <div className="ci-box">{cl[item.key] && '✓'}</div>
                <div className="ci-lbl">{item.label}</div>
                <div className="ci-pts">{item.weight}pts</div>
              </div>
            ))}
          </div>
          ...
          {missing.length>0 && (
            <div className="mand-warn" style={{'--w-bg':theme.wBg, '--w-border':theme.wBorder, '--w-text':theme.wText, '--w-ul':theme.wUl}}>
              <div><strong>Required for Final:</strong><ul>{missing.map(l=><li key={l}>{l}</li>)}</ul></div>
            </div>
          )}

          <div className="sc-blk">
            <div className="sc-label">Setup Quality Score</div>
            <div className="sc-n" style={{color:barColor}}>{score}</div>
            <div className="sc-bg"><div className="sc-fill" style={{width:`${score}%`, background:barColor}}/></div>
            <div className="q-tag" style={{background:score>=75? (score>=90?'#DCFCE7':'#E0F2FE') : '#FEE2E2', color:barColor}}>
              Grade {grade}
            </div>
          </div>
        </div>
        ...
        {score >= 75 && (
          <div className="card">
            <div className="form-sec">Result Analysis</div>
            <div className="g2">
              <div className="field">
                <label>Outcome</label>
              ...
              {result==='Win' && (
                <div className="field">
                  <label>R Multiple Earned</label>
                  <input type="number" value={rMult} onChange={e=>setRMult(e.target.value)} step="0.1" placeholder="2.5"/>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="card">
          ...
          <textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={4} placeholder="Psychology, observations, etc." style={{width:'100%'}}/>
        </div>
      </div>
        ...
        <div className="m-card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 12 }}>
            <div className="field">
              ...
              <input value={pair} onChange={e=>setPair(e.target.value.toUpperCase())} placeholder="e.g. BTCUSD" 
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 12 }}>
              ...
              {result==='Win' && (
                <div className="field">
                  ...
                  <input type="number" value={rMult} onChange={e=>setRMult(e.target.value)} placeholder="2.5" 
                         style={{ width: '100%', padding: '12px', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--m-border)', borderRadius: '12px', color: '#fff' }}/>
                </div>
              )}
            </div>
